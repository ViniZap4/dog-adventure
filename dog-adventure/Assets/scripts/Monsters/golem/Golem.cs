...
private void Update()
	...
	void StateManager()
	{
		...
		switch (selfState)
		{
			case monsterState.ALERT:
				...
				//update vision
				fieldOfView.currentAngle = Mathf.Lerp(fieldOfView.currentAngle, (fieldOfView.angle * 3f), 1f * Time.deltaTime);
				fieldOfView.currentRadius = Mathf.Lerp(fieldOfView.currentRadius, (fieldOfView.radius * 1.1f), 1f * Time.deltaTime);
				StopAgentMovement();
	...
	IEnumerator PATROL()
	{
		...
		while (_GameManager.golemWayPoint.ToArray().Length <= 0)
		{
			print("Golem point still don't load ");
			yield return new WaitForSeconds(0.2f);
		}

		idWaypoint = Random.Range(0, _GameManager.golemWayPoint.ToArray().Length);
		destination = _GameManager.golemWayPoint[idWaypoint].position;
		agent.destination = destination;
